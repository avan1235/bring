...
import `in`.procyk.savvry.ui.components.textfield.TextField
...
internal fun RowScope.AddItemTextField(
    ...
    animationColor: Color = SavvryAppTheme.colors.primary,
    backgroundAnimationColor: Color = SavvryAppTheme.colors.background,
    ...
    InputTextField(
        ...
        brush = when {
            !loading -> SolidColor(SavvryAppTheme.colors.primary)
            progress < 0.5f -> {
...
private fun RowScope.InputTextField(
    ...
    brush: Brush = SolidColor(SavvryAppTheme.colors.primary),
) {
    TextField(
        value = value,
